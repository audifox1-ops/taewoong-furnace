...
export function GasUploadPage() {
  ...
  const { data: uploadHistory } = useQuery({
    ...
    queryFn: () => api.get('/gas-readings/upload-history').then(r => Array.isArray(r.data) ? r.data : r.data?.data ?? []),
  })

  const { data: furnaces } = useQuery({
    queryKey: ['furnaces'],
    queryFn: () => api.get('/furnaces').then(r => Array.isArray(r.data) ? r.data : r.data?.data ?? []),
  })
